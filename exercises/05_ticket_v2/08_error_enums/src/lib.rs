...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError(des)) => Ticket::new(title, des, status).unwrap(),
...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() || description.len() > 500 {
            return Err(TicketNewError::DescriptionError("Description not provided".to_string()));
        }
